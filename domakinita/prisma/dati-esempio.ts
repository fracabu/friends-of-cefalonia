import type { PrismaClient, ContractType, PropertyType } from '@prisma/client'
...
/**
 * Riempie il database con gli annunci di esempio.
 *
 * Sta in un modulo a parte, e riceve il client invece di crearselo, perché
 * serve in due posti: da riga di comando con `pnpm db:seed`, e dalla route
 * protetta che permette di popolare la produzione senza avere un computer
 * sotto mano.
 *
 * Le fotografie e le traduzioni si inseriscono in blocco: dalla funzione
 * serverless al database ogni andata e ritorno costa, e settecento inserimenti
 * uno per volta sfonderebbero il tempo massimo di esecuzione.
 */
export async function popola(db: PrismaClient, log: (messaggio: string) => void = console.log) {
  const console = { log } as { log: (m: string) => void }
  ...
  await db.listingImage.createMany({ data: fotografie })
