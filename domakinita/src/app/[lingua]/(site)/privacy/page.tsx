...
export async function generateMetadata({
  ...
  return { title: d.statiche.privacyTitolo }
}
...
export default async function Pagina({ params }: { params: Promise<{ lingua: string }> }) {
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-16">
      <h1 className="text-3xl font-semibold text-ink-900">{d.statiche.privacyTitolo}</h1>
      <div className="mt-6 space-y-4 text-ink-700">
        <p>{d.statiche.privacy1}</p>
        <p>{d.statiche.privacy2}</p>
      </div>
    </div>
  )
}
